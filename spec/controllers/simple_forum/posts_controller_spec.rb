...
describe SimpleForum::PostsController do
  ...
  describe "GET 'show'" do
    it "should redirect to topic last page with post anchor" do
      ...
      response.should redirect_to(forum_topic_path(@forum, @topic, :page => @post.on_page, :anchor => "post-#{@post.id}"))
    end
  end
  ...
  context "for signed in user" do
    ...
    describe "POST 'create'" do
      describe "with valid params" do
        it "should redirect to topic last page with post anchor and set flash[:notice]" do
          post :create, :forum_id => @forum.to_param, :topic_id => @topic.to_param, :post => FactoryGirl.attributes_for(:post)
          flash[:notice].should_not be_blank
          flash[:alert].should be_blank
          response.should redirect_to(forum_topic_path(@forum, @topic, :page => assigns(:post).on_page, :anchor => "post-#{assigns(:post).id}"))
        end
      end
      describe "with invalid params" do
        ...
        it "should redirect to topic last page with recent post anchor and set flash[:alert]" do
          post :create, :forum_id => @forum.to_param, :topic_id => @topic.to_param, :post => FactoryGirl.attributes_for(:post).inject({}) { |h, (k, v)| h[k] = nil; h }
          flash[:notice].should be_blank
          flash[:alert].should_not be_blank
          response.should redirect_to(forum_topic_path(@forum, @topic, :page => @topic.last_page, :anchor => (@topic.recent_post ? "post-#{@topic.recent_post.id}" : nil)))
        end
      end
    end
    ...
    describe "GET 'edit'" do
      ...
      context "with id own post posted more than #{SimpleForum.minutes_for_edit_post} minutes ago" do
        before(:each) do
          @old_post = FactoryGirl.create(:post, :topic => @topic, :user => @user, :created_at => Time.now - (SimpleForum.minutes_for_edit_post.minutes + 10.minutes))
        end
    ...
    describe "PUT 'update'" do
      context "with id own post posted less than #{SimpleForum.minutes_for_edit_post} minutes ago and valid params" do
        it "should redirect to topic last page with post anchor and set flash[:notice]" do
          put :update, :forum_id => @forum.to_param, :topic_id => @topic.to_param, :id => @post.to_param, :post => FactoryGirl.attributes_for(:post)
          flash[:notice].should_not be_blank
          flash[:alert].should be_blank
          response.should redirect_to(forum_topic_path(@forum, @topic, :page => assigns(:post).on_page, :anchor => "post-#{assigns(:post).id}"))
        end
    ...
    describe "DELETE 'delete'" do
      ...
      context "with id own post posted more than #{SimpleForum.minutes_for_delete_post} minutes ago" do
        before(:each) do
          @old_post = FactoryGirl.create(:post, :topic => @topic, :user => @user, :created_at => Time.now - (SimpleForum.minutes_for_delete_post.minutes + 10.minutes))
        end
        it "should redirect back and set flash['alert']" do
